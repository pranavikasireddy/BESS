...
- **Stochastic optimisation** — deliberately not used, even though it's theoretically the more correct approach for the aFRR-commitment decision specifically (capacity is committed before real-time uncertainty resolves, which is exactly the two-stage decide-then-observe structure stochastic optimisation is built for). Rejected for this build because it needs a credible price-scenario generator as a prerequisite (a project of its own), the problem size grows quickly with scenario count, and the marginal insight per hour of effort is poor for a portfolio project relative to a deterministic rolling-horizon LP re-solved against genuinely known prices. Named here rather than silently omitted.
